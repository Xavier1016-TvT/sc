    ...
    <div className="space-y-4">
      ...
      {!samplePassed ? (
        <div className="card bg-amber-50 border-amber-200 text-amber-800 text-sm">
          订单贴样结果为「通过」后，方可编辑工序、出货不良统计、问题备注和出货情况。
        </div>
      ) : (
        <>
          <CollapsibleSection
            title="生产工序记录"
            subtitle={`贴片 ${getSubProjectCumulative(sub)} · 测试 ${getSubProjectTestCumulative(sub)}`}
          >
            <div className="pt-4">
              <ProcessRecordsTable
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <Stat label="已出货数量" value={`${getSubProjectShipped(sub)} 个`} />
